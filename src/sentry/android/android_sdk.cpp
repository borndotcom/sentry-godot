...
String AndroidSDK::capture_error(const String &p_type, const String &p_value, Level p_level, const Vector<StackFrame> &p_frames) {
	ERR_FAIL_NULL_V(android_plugin, String());
	// Stub: backport from PR #169 does not implement capture_error.
	// Fall back to capturing the type+value as a message.
	return android_plugin->call(ANDROID_SN(captureMessage), p_value, p_level);
}
